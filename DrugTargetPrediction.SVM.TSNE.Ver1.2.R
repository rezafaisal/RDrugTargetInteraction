...
for(i in 1:positive_count){
  ...
  positive_temp = read.csv(positive_file)
...
for(i in 1:negative_count){
  ...
  negative_temp = read.csv(negative_file)[,1:1484]
...
positive_negative_matrix <- as.matrix(positive_negative_unique[,-1484])
...
sleeptime<-5; 
for (it in c(1484,2968,4452)) {
  for (svm_j in 1:svm_count) {
    data.tsne <- Rtsne(positive_negative_matrix,dims=2,PCA=T,max_iter=it); 
    
    result<-vector(); 
    for (svm_i in 1:cross_count) {
      #result[svm_i]<-cross(ksvm(class~.,positive_negative_unique,cross=cross_count));
      result[svm_i]<-cross(ksvm(class~.,cbind(data.tsne$Y,positive_negative_unique[,1484,drop=F]),cross=cross_count));
    }; 
    
    iterasi = it
    mean_result = mean(result)
    result_temp = cbind(iterasi, rbind(result), mean_result)
    if(!exists("classification_result")){
...
write.csv(classification_result, paste0("classification_result.tsne.",test_number,".csv"), row.names = FALSE)
