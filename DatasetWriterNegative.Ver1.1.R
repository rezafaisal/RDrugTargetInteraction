...
file_drug_target_matrix = "matrix_index_negative.test.csv"
...
for(item_index in dataset_index_start:dataset_index_end) {
  ...
  drug_id = i
  target_id = j
  drug_target_feature_vector = cbind(drug_feature_vector[i,], target_feature_vector[j,], class, drug_id, target_id)
  
  if(instance_count > data_per_file){
    write.csv(drug_target_dataset, paste0("drug_target_dataset_negative.",file_index,".",file_sub_index,".csv"), row.names = FALSE, quote= FALSE)
